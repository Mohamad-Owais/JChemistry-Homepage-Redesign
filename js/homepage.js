...
const JChemHomepage = {
  ...
  courses: [
    ...
    {
      id: 7,
      title: "Analytical Chemistry — Tools, Techniques & Data",
      tutor: "Jyoti Rathi",
    ...
    {
      id: 8,
      title: "Quantum Chemistry Bootcamp",
      tutor: "Jyoti Rathi",
    ...
    {
      id: 9,
      title: "Reaction Mechanisms Revision Sprint",
      tutor: "Jyoti Rathi",
    ...
    {
      id: 10,
      title: "Coordination Chemistry Problem Solving",
      tutor: "Jyoti Rathi",
  ...
  updateGrid() {
    ...
    if (!grid) return;
    const results = this.filterCourses(
      this._state.query,
      this._state.activeChip,
    );
    const shouldLimit =
      !this._state.showAllCourses &&
      !this._state.query.trim() &&
      this._state.activeChip === "All";
    const visibleResults = shouldLimit
      ? results.slice(0, this.featuredCourseLimit)
      : results;
    grid.innerHTML = visibleResults.length
      ? this.renderCourseCards(visibleResults)
  ...
  initViewAllCourses() {
    const viewAllLink = document.querySelector(
      '.courses-section__header a[href="#"]',
    );
    if (!viewAllLink) return;

    viewAllLink.addEventListener("click", (e) => {
      e.preventDefault();
      this._state.showAllCourses = true;
      this.updateGrid();
      document
  ...
  init() {
    ...
    if (courseGrid)
      courseGrid.innerHTML = this.renderCourseCards(
        this.courses.slice(0, this.featuredCourseLimit),
      );

    const categoriesGrid = document.getElementById("categories-grid");
    if (categoriesGrid)
      categoriesGrid.innerHTML = this.renderCategories(this.categories);

    this.initChips();
    this.initHomepageSearch();
    this.initViewAllCourses();
  },
};
